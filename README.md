...
```
index.html                   # Hlavní stránka – seznam všech sázek, stats bar, favicon
status.js                    # Sdílená logika: status banner, odpočet, vyhodnocení
Herberk_Salonek_logo.jpg     # Logo – favicon + og:image pro sdílení
...
- `<title>` a OG meta tagy v `<head>` (název sázky, popis)
- Nadpis (`<h1>`)
...
Pokud má sázka pevný termín vyhodnocení, přidej atribut `data-deadline` ve formátu `YYYY-MM-DD` — `status.js` zobrazí odpočet dní přímo ve status banneru:

```html
<li data-status="waiting" data-deadline="2026-06-30" data-file="vsazka-nova.html">
```
...
Soubor `status.js` obsluhuje všechny sázkové podstránky. Načte `index.html`, přečte stav sázky a:

1. Vykreslí **status banner** (pulsující tečka, název stavu, popis)
2. Pokud je na `<li>` atribut `data-deadline` a stav není `resolved`/`closed`, zobrazí **odpočet dní do termínu** uvnitř banneru
3. Pokud je stav `resolved`, spustí **vyhodnocení výsledků** (zvýraznění výherců)
...
Každý HTML soubor obsahuje:
- `<meta name="description">` – popis stránky pro vyhledávače
- Open Graph tagy (`og:title`, `og:description`, `og:image`, `og:type`) – náhled při sdílení odkazu na sociálních sítích
- `<meta name="twitter:card" content="summary_large_image">` – velký náhled na X/Twitter

Obrázek pro náhled: `Herberk_Salonek_logo.jpg`. Meta tag používá relativní cestu – pro plnou kompatibilitu s Facebookem je potřeba absolutní URL:
```html
<meta property="og:image" content="https://username.github.io/Herberk-vsazky/Herberk_Salonek_logo.jpg">
```

Favicon je nastaven v `index.html`:
```html
<link rel="icon" href="Herberk_Salonek_logo.jpg" type="image/jpeg">
```
